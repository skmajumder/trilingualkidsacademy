<?php
$homeBookTrialSectionMataQuery = [
	'key'     => 'section_type',
	'compare' => '=',
	'value'   => 'demo',
];

$homeBookTrialSectionArgs = [
	'post_type'           => 'section',
	'posts_per_page'      => 1,
	'meta_key'            => 'section_type',
	'meta_value'          => 'demo',
	'meta_query'          => $homeBookTrialSectionMataQuery,
	'ignore_sticky_posts' => true
];
$homeBookTrialSection     = new WP_Query( $homeBookTrialSectionArgs );
if ( $homeBookTrialSection->post_count > 0 ):
	if ( $homeBookTrialSection->have_posts() ): while ( $homeBookTrialSection->have_posts() ): $homeBookTrialSection->the_post();
		?>
        <section class="tf-section tf-register" id="<?php echo esc_attr( $post->post_name ); ?>">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/images/background/bg-register.png" alt="Image"
                 class="feature-register">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="title-heading st-5">
                            <div class="sub-heading clr-pri-3 f-mulish">
                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                     width="77"
                                     height="30" viewBox="0 0 77 30">
                                    <g>
                                        <image width="77" height="30"
                                               xlink:href="data:img/png;base64,"></image>
                                    </g>
                                </svg>
	                            <?php
	                            if ( function_exists( 'get_field' ) ) {
		                            $subHeadline = get_field( 'book_free_demo_sub_headline' );
		                            if ( $subHeadline ) {
			                            echo '<span class="inner-sub st-1">' . esc_html( $subHeadline ) . '</span>';
		                            }
	                            }
	                            ?>
                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                     width="77"
                                     height="30" viewBox="0 0 77 30">
                                    <g>
                                        <image width="77" height="30"
                                               xlink:href="data:img/png;base64,"></image>
                                    </g>
                                </svg>
                            </div>
	                        <?php
	                        if ( function_exists( 'get_field' ) ) {
		                        $headline = get_field( 'book_free_demo_headline' );
		                        if ( $headline ) {
			                        echo '<h2 class="title clr-pri-2">' . esc_html( $headline ) . '</h2>';
		                        }
	                        }
	                        ?>
                        </div>
                    </div>
                    <div class="col-12">
                        <div class="fl-register wow fadeInUp   animated" data-wow-delay="0.3ms"
                             data-wow-duration="1000ms"
                             style="visibility: visible; animation-duration: 1000ms; animation-delay: 0.3ms;">
                            <div class="row text-center">
                                <div class="col-lg-6 offset-lg-3">
									<?php the_content(); ?>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
	<?php
	endwhile; endif;
endif;
wp_reset_postdata();
?>