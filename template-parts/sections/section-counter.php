<?php
$homeCounterSectionMataQuery = [
	'key'     => 'section_type',
	'compare' => '=',
	'value'   => 'counter',
];

$homeCounterSectionArgs = [
	'post_type'           => 'section',
	'posts_per_page'      => 1,
	'meta_key'            => 'section_type',
	'meta_value'          => 'counter',
	'meta_query'          => $homeCounterSectionMataQuery,
	'ignore_sticky_posts' => true
];
$homeCounterSection     = new WP_Query( $homeCounterSectionArgs );
if ( $homeCounterSection->post_count > 0 ):
	if ( $homeCounterSection->have_posts() ): while ( $homeCounterSection->have_posts() ): $homeCounterSection->the_post();
		?>
        <section <?php post_class( 'tf-section tf-counter' ); ?> id="<?php echo esc_attr( $post->post_name ); ?>">
            <div class="container">
                <div class="row">

                    <!-- :: Student Enroll ::-->
					<?php
					if ( function_exists( 'get_field' ) ):
						$studentEnroll = get_field( 'counter_info_students_total_student_number' );
						$studentText = get_field( 'counter_info_students_text' );
						if ( $studentEnroll && $studentText ): ?>
                            <div class="col-xl-3 col-lg-3 col-md-3 col-12">
                                <div class="sc-fun-fact st-1 themesflat-counter wow fadeIn animated"
                                     data-wow-delay="0.2ms"
                                     data-wow-duration="1300ms">
                                    <div class="box-icon">
                                        <svg data-name="Fun fact" xmlns="http://www.w3.org/2000/svg"
                                             xmlns:xlink="http://www.w3.org/1999/xlink" width="66" height="66"
                                             viewBox="0 0 66 66">
                                            <g id="_01" data-name="01">
                                                <g>
                                                    <image id="Icon-2" data-name="Icon" width="66" height="66"
                                                           xlink:href="data:img/png;base64,"/>
                                                </g>
                                            </g>
                                        </svg>
                                    </div>
                                    <div class="box-content box">
                                        <div class="number-content clr-pri-1">
                                        <span class="number" data-speed="3000"
                                              data-to="<?php echo esc_attr( $studentEnroll ); ?>"
                                              data-inviewport="yes"><?php echo esc_html( $studentEnroll ); ?></span>+
                                        </div>
                                        <p class="clr-pri-1">
											<?php echo esc_html( $studentText ); ?>
                                        </p>
                                    </div>
                                </div>
                            </div>
						<?php endif; endif; ?>
                    <!-- :: Student Enroll ::-->

					<?php
					if ( function_exists( 'get_field' ) ):
						$classNumber = get_field( 'counter_info_classes_total_classe_number' );
						$classText = get_field( 'counter_info_classes_text' );
						if ( $classNumber && $classText ): ?>
                            <div class="col-xl-3 col-lg-3 col-md-3 col-12">
                                <div class="sc-fun-fact st-2 themesflat-counter wow fadeIn animated"
                                     data-wow-delay="0.2ms"
                                     data-wow-duration="1300ms">
                                    <div class="box-icon style2">
                                        <svg data-name="Fun fact" xmlns="http://www.w3.org/2000/svg"
                                             xmlns:xlink="http://www.w3.org/1999/xlink" width="62" height="60"
                                             viewBox="0 0 62 60">
                                            <g id="_02" data-name="02">
                                                <g>
                                                    <image id="dictionary" width="62" height="60"
                                                           xlink:href="data:img/png;base64,"/>
                                                </g>
                                            </g>
                                        </svg>
                                    </div>
                                    <div class="box-content box">
                                        <div class="number-content clr-pri-1">
                                        <span class="number" data-speed="3000"
                                              data-to="<?php echo esc_attr( $classNumber ); ?>"
                                              data-inviewport="yes"><?php echo esc_html( $classNumber ); ?></span>+
                                        </div>
                                        <p class="clr-pri-1"><?php echo esc_html( $classText ); ?></p>
                                    </div>
                                </div>
                            </div>
						<?php endif; endif; ?>

					<?php
					if ( function_exists( 'get_field' ) ):
						$teacherNumber = get_field( 'counter_info_teachers_total_teacher' );
						$teacherText = get_field( 'counter_info_teachers_text' );
						if ( $teacherNumber && $teacherText ): ?>
                            <div class="col-xl-3 col-lg-3 col-md-3 col-12">
                                <div class="sc-fun-fact st-3 themesflat-counter wow fadeIn animated"
                                     data-wow-delay="0.2ms"
                                     data-wow-duration="1300ms">
                                    <div class="box-icon">
                                        <svg data-name="Fun fact" xmlns="http://www.w3.org/2000/svg"
                                             xmlns:xlink="http://www.w3.org/1999/xlink" width="61" height="55"
                                             viewBox="0 0 61 55">
                                            <g id="_03" data-name="03">
                                                <g>
                                                    <image id="online-class" width="61" height="55"
                                                           xlink:href="data:img/png;base64,"/>
                                                </g>
                                            </g>
                                        </svg>
                                    </div>
                                    <div class="box-content box">
                                        <div class="number-content clr-pri-1">
                                        <span class="number" data-speed="3000"
                                              data-to="<?php echo esc_attr( $teacherNumber ); ?>"
                                              data-inviewport="yes"><?php echo esc_html( $teacherNumber ); ?></span>+
                                        </div>
                                        <p class="clr-pri-1"><?php echo esc_html( $teacherText ); ?></p>
                                    </div>
                                </div>
                            </div>
						<?php endif; endif; ?>

					<?php
					if ( function_exists( 'get_field' ) ):
						$experienceNumber = get_field( 'counter_info_experience_total_years_experience' );
						$experienceText = get_field( 'counter_info_experience_text' );
						if ( $experienceNumber && $experienceText ): ?>
                            <div class="col-xl-3 col-lg-3 col-md-3 col-12">
                                <div class="sc-fun-fact st-3 themesflat-counter mgbt-0 wow fadeIn animated"
                                     data-wow-delay="0.2ms" data-wow-duration="1300ms">
                                    <div class="box-icon">
                                        <svg data-name="Fun fact" xmlns="http://www.w3.org/2000/svg"
                                             xmlns:xlink="http://www.w3.org/1999/xlink" width="61" height="59"
                                             viewBox="0 0 61 59">
                                            <g id="_04" data-name="04">
                                                <g>
                                                    <image id="notes" width="61" height="59"
                                                           xlink:href="data:img/png;base64,"/>
                                                </g>
                                            </g>
                                        </svg>
                                    </div>
                                    <div class="box-content box">
                                        <div class="number-content clr-pri-1">
                                        <span class="number" data-speed="3000"
                                              data-to="<?php echo esc_attr( $experienceNumber ); ?>"
                                              data-inviewport="yes"><?php echo esc_html( $experienceNumber ); ?></span>+
                                        </div>
                                        <p class="clr-pri-1"><?php echo esc_html( $experienceText ); ?></p>
                                    </div>
                                </div>
                            </div>
						<?php endif; endif; ?>
                </div>
            </div>
        </section>
	<?php
	endwhile; endif;
endif;
wp_reset_postdata();
?>